...
import { RelationEnum, useOperatePermissions } from './hooks/useOperatePermissions';
...
export const MessageConsole: React.FC<MessageConsoleProps> = (props) => {
  const { Input, OperateContainer = FollowRequestButtonGroup, warnText } = props;
  ...
  const getWarnText = () => {
    ...
    if (toChatTargetUser === RelationEnum.canMessage || !permissions['user:chat']) return '';
    // 对方权限是follower or 对方权限是friend 我只被对方关注
    if (
      permissions['user:chat'].value === 'follower' ||
    ...
    if (
      targetUserPermissions.permissions['user:chat'].value === 'following' ||
      (permissions['user:chat'].value === 'friend' && follow_status === 'following')
    ) {
  ...
  const getShowFollow = () => {
    ...
    if (permissions['user:chat']) {
      if (
        (permissions['user:chat'].value === 'follower' &&
          (follow_status === '' || follow_status === 'follower')) ||
        (permissions['user:chat'].value === 'friend' && follow_status === 'follower')
      )
        return true;
      return false;
  ...
  if (isGroup || toChatTargetUser === RelationEnum.canMessage) return <>{Input || <MessageInput />}</>;
